...
# define a set over which to try different values of the regularization parameter (number of neighbors)
knn_parm_df <- tibble(neighbors = seq(1,10))
...
# note that I could not get anything past this point to actually run for svm
# but I still filled in the code as it would be
...
svm_knn <- finalize_workflow(svm_wf,
                               svm_best_acc
)
print('*********** TREE **************')
...
# not including svm_ans because I could not get svm_res to run so I dont have svm_ans
all_ans <- bind_rows(logit_ans,tree_ans,nnet_ans,knn_ans)
# standardize the tree estimate to go with the rest of the estimates
all_ans <- all_ans %>%
  mutate(.estimate = coalesce(.estimate.x, .estimate)) %>%  # Merge data from estimate.x into estimate
  select(-.estimate.y, -.estimate.x) %>%  # Drop estimate.y and estimate.x columns
  distinct()
